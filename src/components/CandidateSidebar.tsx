...
function CandidateSidebar({ isCollapsed, setIsCollapsed }: CandidateSidebarProps) {
  ...
  return (
    <aside
      className={`fixed top-0 z-50 flex h-screen flex-col border-white/10 bg-[linear-gradient(180deg,#15184c_0%,#111444_60%,#0d1038_100%)] text-white transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-white/10 px-5 pb-5 pt-5">
        <button
          ...
          className={`flex items-center transition hover:opacity-90 ${isCollapsed ? "justify-center" : "gap-4"}`}
        >
          <div className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-[16px] bg-gradient-to-br from-[#7c4dff] to-[#a855f7] shadow-[0_12px_28px_rgba(124,77,255,0.35)]">
            <Sparkles size={20} />
          </div>
          {!isCollapsed && (
          ...
          <div className="mb-4 flex rounded-[18px] border border-white/10 bg-white/[0.03] p-1">
            {[
              ...
              <button
                ...
                className={`flex-1 rounded-[14px] px-2 py-2.5 text-[12px] font-semibold transition ${
                  language === lang.key
                    ? "bg-[rgba(99,102,241,0.45)] text-[#dce3ff]"
                    : "text-[#9ca3c5] hover:text-white"
                }`}
              >
                {lang.label}
              </button>
        ...
        <button
          ...
          className={`flex w-full items-center rounded-[18px] text-[17px] font-semibold text-[#b7bddb] transition hover:bg-white/[0.04] hover:text-white ${
            isCollapsed
              ...
              : `gap-3 px-4 py-3 ${isRTL ? "text-right" : "text-left"}`
          }`}
        >
          <LogOut size={21} />
